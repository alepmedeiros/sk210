...
type
  TJAidlDeviceService_Stub = class(TJavaGenericImport<JAidlDeviceService_StubClass, JAidlDeviceService_Stub>)
  public
    class function asInterface(binder: JIBinder): JAidlDeviceService; cdecl; // Adicionando o método manualmente
  end;
  ...
  public
    ...
    procedure onBindingDied(name: JComponentName); cdecl;  // Implementação necessária
    procedure onNullBinding(name: JComponentName); cdecl;  // Implementação necessária
  ...
  private
    FContext: JContext;
    FDeviceServiceConnection: TMyServiceConnection;
    FIsBind: Boolean;
...
begin
  // Vinculando o serviço de dispositivo
...
begin
  ...
  // Chama o callback de desconexão
...
begin
  // Tratar a morte da ligação (pode ser deixado como um log simples)
...
begin
  // Tratar uma ligação nula (pode ser deixado como um log simples)
...
begin
  if Assigned(FDeviceServiceConnection) then
    Result := FDeviceServiceConnection.DeviceService
  else
    Result := nil;
end;
...
begin
  ...
  // Inicia a conexão com o serviço
  FDeviceServiceConnection := TMyServiceConnection.Create(
  ...
  // Vincular ao serviço
  Result := TAndroidHelper.Context.bindService(Intent, FDeviceServiceConnection, TJContext.JavaClass.BIND_AUTO_CREATE);
end;
...
begin
  if FIsBind then
  begin
    FContext.unbindService(FDeviceServiceConnection);
    FIsBind := False;
  end;
end;
...
begin
  if Assigned(FDeviceServiceConnection) and Assigned(FDeviceServiceConnection.DeviceService) then
    Result := TJAidlPrinter_Stub.JavaClass.asInterface(FDeviceServiceConnection.DeviceService.getPrinter)
  else
    Result := nil;
end;
...
class function TJAidlDeviceService_Stub.asInterface(
  binder: JIBinder): JAidlDeviceService;
begin
